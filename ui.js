...
VIEWS.book = {
  html(params) {
    ...
    const act = activeFighters().sort((x, y) => y.elo - x.elo);
    ...
    const opts = sel => `<option value="">— select —</option>` + act.map(f =>
      `<option value="${f.id}" ${sel === f.id ? 'selected' : ''}>${esc(f.name)} (${overall(f)} · ${f.elo} · ${recordStr(f)})${isChampion(f) ? ' 🏆' : ''}</option>`).join('');
    ...
    if (a && b && a.id !== b.id) {
      ...
      const row = (lab, va, vb, hi) => {
        const aAdv = hi === 'high' ? va > vb : va < vb;
        return `<div class="l ${va !== vb && aAdv ? 'adv' : ''}">${va}</div><div class="m">${lab}</div><div class="r ${va !== vb && !aAdv ? 'adv' : ''}">${vb}</div>`;
      };
    ...
    return `<div class="grid">
      <div class="panel">
        <h2>Book a Fight</h2>
        <div class="grid grid-2">
          <label class="fld"><span style="color:#ff9ea2">Red Corner</span><select id="bk-a">${opts(bookSel.a)}</select></label>
          <label class="fld"><span style="color:#a4c6f4">Blue Corner</span><select id="bk-b">${opts(bookSel.b)}</select></label>
        </div>
        <div class="grid grid-2">
      ...
      ${a && b && a.id === b.id ? '<div class="panel corner-red">A fighter cannot fight themselves.</div>' : ''}
      ${tapeHtml}
      ${oddsHtml}
      <div class="btn-row">
  ...
  wire() {
    $('#bk-a').addEventListener('change', e => { bookSel.a = e.target.value || null; render('book'); });
    $('#bk-b').addEventListener('change', e => { bookSel.b = e.target.value || null; render('book'); });
    ...
    $('#bk-sim').addEventListener('click', () => {
      ...
      autosave();
      bookSel = { a: null, b: null, title: false, rounds: 3, event: '' };
